...
#ifndef header_css_selector
#define header_css_selector
...
//: Cascading Style Sheet (CSS) selector.
...
//! Attributes:
public:
	//: Returns the path elements required to match the selector.
	const std::vector<CL_String> &get_path_elements() const;

//! Operations:
public:
	//: Returns true if selector matches a given element path.
	bool select(const std::vector<CL_StringRef> &element, int &out_specificity) const;

	//: Adds another path element to selector.

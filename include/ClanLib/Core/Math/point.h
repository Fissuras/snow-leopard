...
public:
	...
	explicit CL_Point(const CL_Pointd& p);
...
public:
		...
		float angle) const
	{
		...
		const float nw = (float)(atan2((float)r.y, (float)r.x) + ((angle + 180) * CL_PI / 180));
		r.x = (int)((sin(1.5 * CL_PI - nw) * c) + 0.5) + hotspot.x;
		r.y = -(int)((sin(nw) * c) + 0.5) + hotspot.y;
		#else
		const float c = (float) std::sqrt((float)r.x*(float)r.x + (float)r.y*(float)r.y);
		const float nw = (float)(std::atan2((float)r.y, (float)r.x) + ((angle + 180) * CL_PI / 180));
 		r.x = (int)((std::sin(1.5 * CL_PI - nw) * c) + 0.5) + hotspot.x;
	...
	{
		#ifdef _MSC_VER
    	return int(sqrt(double((x-p.x)*(x-p.x) + (y-p.y)*(y-p.y))) + 0.5);
		#else
    	return int(std::sqrt(double((x-p.x)*(x-p.x) + (y-p.y)*(y-p.y))) + 0.5);
		#endif
	}
...
public:
		...
		float angle) const
	{
		...
		const float nw = (float)((float)atan2(r.y, r.x) + ((angle + 180) * CL_PI / 180));

		r.x = ((float)(sin(1.5 * CL_PI - nw)) * c) + hotspot.x;
		r.y = -((float)(sin(nw)) * c) + hotspot.y;
		#else
 		const float c = (float)(std::sqrt(r.x*r.x + r.y*r.y));

		const float nw = (float)((float)std::atan2(r.y, r.x) + ((angle + 180) * CL_PI / 180));

		r.x = ((float)(std::sin(1.5 * CL_PI - nw)) * c) + hotspot.x;
...
//: 2D (x,y) double floating point point structure.
//- !group=Core/Math!
//- !header=core.h!
class CL_Pointd
{
...
public:
	...
	//param angle: The amount of degrees to rotate by, clockwise.
	CL_Pointd rotate(
		const CL_Pointd &hotspot,
		double angle) const
	{
		// Move the hotspot to 0,0
		CL_Pointd r(x - hotspot.x, y - hotspot.y);
	...
	//: Return the distance to another point.
	//param CL_Pointf &p: The other point.
	double distance( const CL_Pointd &p ) const
	{
		#ifdef _MSC_VER
    	return (float) sqrt((x-p.x)*(x-p.x) + (y-p.y)*(y-p.y));
		#else
	...
	//: Translate point.
	CL_Pointd &operator+=(const CL_Pointd &p)
	{ x += p.x; y += p.y; return *this; }
	...
	//: Point + Point operator.
	CL_Pointd operator+(const CL_Pointd &p) const
	{ return CL_Pointd(x + p.x, y + p.y); }
	...
	//: Point == Point operator (deep compare)
	bool operator==(const CL_Pointd &p) const
	{ return (x == p.x) && (y == p.y); }
...
inline CL_Point::CL_Point(const CL_Pointd& p)
